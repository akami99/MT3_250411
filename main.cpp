...
const char kWindowTitle[] = "LE2B_01_アカミネ_レン_MT3_01-01";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	const Vector3 kLocalVertices[3]{ {0.0f, 0.5f, 0.0f}, {-0.5f, -0.5f, 0.0f}, {0.5f, -0.5f, 0.0f} };
	...
	while (Novice::ProcessMessage() == 0) {
		...
		// WSキーで前後に、ADキーで左右に三角形を動かす。Y軸回転をさせる。という処理をここに書く、translateとrotateの値を変更すれば良い。
		if (keys[DIK_W]) {
			translate.z += 0.1f;
		}
		if (keys[DIK_S]) {
